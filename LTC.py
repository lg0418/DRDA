    ...
    while item_idx < len(item):
        if clock != item[item_idx][0]:    #当前没有item到来
                ...
                for ts in range(ta,ta+td):  #遍历item开始时间
                    ...
                    for item_time in range(1,ta+td-ts+1):  #遍历item持续时间
                        ...
                        if freeblock:
                            ...
                            max_tf_current_item_time_block = [item_time,max(tf),max_tf_block,item_frequency]
            ...
            if diff_path_ts_item_time_max_tf_block:
                ...
                max_tf_path_ts_item_time_block = diff_path_ts_item_time_max_tf_block[0]  #[持续时间，tf,空闲块位置,频谱槽，开始时间，路径编号]
                select_item_time = max_tf_path_ts_item_time_block[0]
